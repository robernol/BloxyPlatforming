...
public class Water : MonoBehaviour 
{
	...
	void OnTriggerEnter(Collider other)
	{
		...
		if(r)
		{
			if(splashSound)
			{
				float volume = other.GetComponent<Rigidbody>().linearVelocity.magnitude/5;
			...
			dragStore.Add (r.gameObject, r.linearDamping);
			angularStore.Add(r.gameObject, r.angularDamping);
			
			//apply new drag values to object
			r.linearDamping = resistance;
			r.angularDamping = angularResistance;
		}
		else if(splashSound)
	...
	void OnTriggerExit(Collider other)
	{
		...
		if(r)
		{
			...
			{
				//restore values
				r.linearDamping = dragStore[r.gameObject];
				r.angularDamping = angularStore[r.gameObject];
